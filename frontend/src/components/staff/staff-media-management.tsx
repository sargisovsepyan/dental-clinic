...
function GalleryGrid({ items, locale, pending, onEdit, onArchive, onRestore }: {
  ...
  return <div className="mt-7 grid gap-5 sm:grid-cols-2 xl:grid-cols-3">{items.map((item) => {
    ...
    return <article key={item.id} className="min-w-0 overflow-hidden rounded-xl border bg-card shadow-sm">
      <ManagedMediaPreview asset={item.image} alt={alt} className="aspect-[4/3]" />
      <div className="p-4">
...
function EntityMediaGrid({ kind, items, locale, pending, onEdit, onRemove }: {
  ...
  return <div className="mt-7 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">{items.map((item) => {
    ...
    return <article key={item._id} className="min-w-0 rounded-xl border bg-card p-4 shadow-sm">
      <ManagedMediaPreview asset={asset} alt={label} className="aspect-[4/3] rounded-lg" />
