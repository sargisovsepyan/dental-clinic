...
import { formatClinicDate, statusClass, statusLabel, snapshotServiceName, snapshotDentistName } from "@/components/staff/staff-appointment-helpers";
...
function AppointmentCard({ item, locale, copy }: { item: StaffAppointment; locale: "hy" | "ru" | "en"; copy: ReturnType<typeof useStaffAuth>["copy"] }) {
  return (
    <article className="rounded-xl border bg-card p-5">
      ...
      <dl className="mt-5 grid grid-cols-2 gap-3 text-sm"><div><dt className="text-xs text-muted-foreground">{copy.date}</dt><dd className="mt-1">{formatClinicDate(item.date, locale)}</dd></div><div><dt className="text-xs text-muted-foreground">{copy.time}</dt><dd className="mt-1">{item.startTime}–{item.endTime}</dd></div><div className="col-span-2"><dt className="text-xs text-muted-foreground">{copy.service}</dt><dd className="mt-1">{snapshotServiceName(item, locale)}</dd></div></dl>
