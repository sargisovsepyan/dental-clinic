...
function CaseCard({ item, locale, pending, onEdit, onReplace, onArchive, onRestore, onWithdraw, onPurge }: {
  item: StaffBeforeAfterCase;
  locale: Locale;
  pending: boolean;
  onEdit: () => void;
  ...
  return <article className="overflow-hidden rounded-xl border bg-card shadow-sm">
    <div className="grid grid-cols-2 gap-px bg-border"><div className="min-w-0 bg-card"><p className="p-2 text-center text-xs font-bold uppercase tracking-wide">{copy.before}</p><ManagedMediaPreview asset={item.beforeImage} alt={`${copy.before}: ${caseTitle(item, locale)}`} className="aspect-[4/3]" /></div><div className="min-w-0 bg-card"><p className="p-2 text-center text-xs font-bold uppercase tracking-wide">{copy.after}</p><ManagedMediaPreview asset={item.afterImage} alt={`${copy.after}: ${caseTitle(item, locale)}`} className="aspect-[4/3]" /></div></div>
...
function BeforeAfterContent() {
  ...
  return <section className="max-w-7xl"><ManagementHeader eyebrow={copy.casesNav} title={copy.beforeAfterTitle} intro={copy.beforeAfterIntro} action={<Button onClick={() => setEditor({ open: true, value: null })} disabled={loading}><Plus aria-hidden="true" />{copy.addCase}</Button>} /><ManagementFeedback value={feedback} onRetry={() => void load().catch(() => undefined)} />
    {loading ? <p role="status" className="mt-8 text-sm text-muted-foreground">{copy.loading}</p> : cases.length === 0 ? <p className="mt-7 rounded-xl border bg-card p-6 text-muted-foreground">{copy.noCases}</p> : <div className="mt-7 grid gap-5 xl:grid-cols-2">{cases.map((item) => <CaseCard key={item.id} item={item} locale={locale} pending={pending} onEdit={() => setEditor({ open: true, value: item })} onReplace={(side) => setReplacement({ item, side })} onArchive={() => setArchiveTarget(item)} onRestore={() => void mutate(() => api.restoreBeforeAfterCase(item.id), { close: () => undefined, success: copy.caseSaved })} onWithdraw={() => setReasonTarget({ item, kind: "withdraw" })} onPurge={() => setReasonTarget({ item, kind: "purge" })} />)}</div>}
