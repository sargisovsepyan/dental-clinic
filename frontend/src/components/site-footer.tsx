  ...
  return (
    <footer className="border-t bg-foreground text-background">
      <div className="site-container grid gap-12 py-14 md:grid-cols-[1.2fr_1fr_1fr] md:py-20">
        <div>
          <Link href={localizedPath(locale)} className="display-type text-2xl">
            <span lang={clinic?.name.lang || (clinic?.name.text ? undefined : "en")}>{clinic?.name.text || "Arelis Dental"}</span>
          </Link>
      ...
      <div className="site-container border-t border-background/15 py-6 text-xs text-background/55">
        © {new Date().getUTCFullYear()} <span lang={clinic?.name.lang || (clinic?.name.text ? undefined : "en")}>{clinic?.name.text || "Arelis Dental"}</span>. {copy.allRights}
      </div>
    </footer>
  );
}
