...
const translations = (fields, rows) => Object.fromEntries(
  locales.map((locale, index) => [
    locale,
    Object.fromEntries(
      fields.map((field, column) => [field, rows[index][column]])
    ),
  ])
);

const weeklySchedule = (flag) => Array.from(
  { length: 7 },
  (_, index) => ({
    dayOfWeek: index + 1,
    [flag]: index < 6,
...
// Kept in lockstep with frontend/test/preview/arelis-catalog.mjs. Preview IDs,
// timestamps and media are deliberately not copied into production.
const serviceRows = [
  ['consultation', 0, 0, 30,
...
const clinicTranslations = translations(
  ['clinicName', 'tagline', 'description', 'address'],
  [
    ['Arelis Dental', 'Arelis Dental — ժամանակակից ատամնաբուժություն ամբողջ ընտանիքի համար', 'Arelis Dental-ը միավորում է ատամնաբուժության հիմնական ուղղությունները՝ կանխարգելումից և բուժումից մինչև էսթետիկ ստոմատոլոգիա, օրթոդոնտիա, պրոթեզավորում և իմպլանտացիա։ Բուժումից առաջ բժիշկը բացատրում է տարբերակները, փուլերը և մոտավոր արժեքը։ Ընտրեք ծառայությունը, ծանոթացեք բժիշկներին և ամրագրեք ձեզ հարմար այցի ժամանակը։', 'Կենտրոն, Երևան՝ Հանրապետության հրապարակի մոտ'],
    ['Arelis Dental', 'Arelis Dental — современная стоматология для всей семьи', 'Arelis Dental объединяет основные направления стоматологической помощи: от профилактики и лечения до эстетической стоматологии, ортодонтии, протезирования и имплантации. Перед началом лечения врач объясняет варианты, этапы и ориентировочную стоимость. Выберите услугу, познакомьтесь с врачами и найдите удобное время для визита.', 'Кентрон, Ереван — рядом с площадью Республики'],
    ['Arelis Dental', 'Arelis Dental — modern dentistry for the whole family', 'Arelis Dental brings together preventive care, tooth treatment, aesthetic dentistry, orthodontics, prosthetics and implant care. Before treatment, your dentist explains the options, stages and estimated costs. Explore our services, meet the team and choose a convenient time for your visit.', 'Kentron, Yerevan — near Republic Square'],
  ]
);

const ArelisDemoData = {
  clinic: {
    ...clinicTranslations.hy,
    translations: clinicTranslations,
    mapUrl: 'https://www.google.com/maps/search/?api=1&query=Republic+Square%2C+Yerevan',
    weeklySchedule: weeklySchedule('isOpen'),
    bookingSettings: {
  ...
  categories: categoryRows.map(([slug, ...names], index) => ({
    slug,
    name: names[0],
    description: '',
  ...
  services: serviceRows.map(([
    ...
    const localized = translations(
      ...
      rows.map(([name, body]) => [
        name,
        `${body.split('։')[0].split('. ')[0].replace(/\.$/, '')}${body.includes('։') ? '։' : '.'}`,
        body,
      ])
    );
    return {
      slug,
      categorySlug: categoryRows[categoryIndex][0],
      ...localized.hy,
      translations: localized,
      priceType: price === 0 ? 'fixed' : 'from',
      priceFrom: price,
      priceTo: null,
      currency: 'AMD',
      durationMinutes,
      imageUrl: '',
      image: null,
      isFeatured: [0, 2, 4, 14].includes(index),
      bookingEnabled: true,
      isActive: true,
      sortOrder: index,
    };
  }),
  dentists: dentistRows.map(([slug, serviceIndexes, ...rows], index) => {
    const localized = translations(
      ['firstName', 'lastName', 'title', 'bio'],
      rows
    );
    for (const entry of Object.values(localized)) {
      entry.specializations = [entry.title];
    }
    return {
      slug,
      ...localized.hy,
      ...
      photo: null,
      languages: ['hy', 'ru', 'en'],
      serviceSlugs: serviceIndexes.map((serviceIndex) => serviceRows[serviceIndex][0]),
      weeklySchedule: weeklySchedule('isWorking'),
      isFeatured: true,
      bookingEnabled: true,
      isActive: true,
      sortOrder: index,
  ...
  staff: {
    ...
    dentist: {
      ...
      role: 'dentist',
      dentistSlug: 'davit-petrosyan',
    },
  },
};

export { ArelisDemoData };
