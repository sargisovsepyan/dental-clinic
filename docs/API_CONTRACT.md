...
Staff listing applies validated server filters, defaults to page 1/limit 50, and sorts by `name ASC, _id ASC`. The staff workspace sends `lifecycle=current` by default; omitting lifecycle at the API retains the backward-compatible unfiltered view. `current` contains established active staff plus pending invitations and excludes deactivated/cancelled accounts; `active`, `pending`, `deactivated`, and `all` are explicit alternatives. The lifecycle filter is mutually exclusive with the legacy `isActive` and `setupComplete` filters. Invitations return a safe staff record, never a setup token. An explicit resend is allowed only for an uncancelled, setup-incomplete invitation: it preserves the original name/email/role, atomically invalidates previous setup links, sends one new link, and never returns the raw token. Clients never replay an uncertain invitation write automatically. Deactivating a pending invite is cancellation without deleting its audit history. Established-account reactivation requires completed setup. Role changes, deactivation, reactivation, and revoke-all-sessions invalidate bearer authorization; self role/deactivation are refused. A successful self session revocation also invalidates the requesting browser.
