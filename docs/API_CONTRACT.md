...
- `POST /auth/reset-password` and `POST /auth/setup-password` atomically consume a hashed, expiring, single-use token. `POST /auth/invitation-context` accepts the same invitation token and returns only the pending employee's non-secret setup context; it never returns or persists the raw token in a response.
- Authentication, staff-administration, audit-administration, authenticated catalog/scheduling/media/consent management, appointment, availability, and all public before/after responses send `Cache-Control: no-store`; authenticated browser clients must also avoid shared framework caches. Public catalog, gallery, and clinic reads retain their public cache policy. Governed before/after reads are deliberately not shared-cacheable so consent withdrawal is reflected on the next request.
...
Staff listing applies validated server filters, defaults to page 1/limit 50, and sorts by `name ASC, _id ASC`. The staff workspace sends `lifecycle=current` by default; omitting lifecycle at the API retains the backward-compatible unfiltered view. `current` contains established active staff plus pending invitations and excludes deactivated/cancelled accounts; `active`, `pending`, `deactivated`, and `all` are explicit alternatives. The lifecycle filter is mutually exclusive with the legacy `isActive` and `setupComplete` filters. Invitations return a safe staff record, never a setup token. Dentist invitations and role changes require an existing dentist profile; admin and receptionist accounts cannot carry that relationship. A partial unique database index enforces at most one non-archived staff account per dentist profile under races. An explicit resend is allowed only for an uncancelled, setup-incomplete invitation: it preserves the original name/email/role/profile relationship, atomically invalidates previous setup links, sends one new link, and never returns the raw token. Clients never replay an uncertain invitation write automatically. `POST /staff/:id/cancel-invitation` is the explicit pending-invitation terminal action: it consumes setup links, revokes authorization, archives the account, and preserves audit history. Established accounts use deactivate/reactivate and require completed setup to restore. Role changes, deactivation, reactivation, dentist-profile changes, and revoke-all-sessions invalidate bearer authorization; self role/deactivation are refused. A successful self session revocation also invalidates the requesting browser. Archiving dentist staff does not unpublish or delete the independent public dentist profile; restoring fails with `409` if another current account has since claimed that profile.
...
| Staff | none | list/detail, invite/resend, role, deactivate/reactivate, revoke sessions (admin) |
...
Appointment states are `pending`, `confirmed`, `checked_in`, `in_progress`, `completed`, `cancelled`, and `no_show`. The normal path is `pending -> confirmed -> checked_in -> in_progress -> completed`. Safe forward shortcuts are also supported: `confirmed -> in_progress|completed` and `checked_in -> completed`; `pending` or `confirmed` may become `no_show`. Backward moves and transitions from terminal states are rejected, and cancellation uses its dedicated route. Reschedule is intentionally limited to `pending` or `confirmed`; once care has begun (`checked_in` or `in_progress`) the occurrence cannot be rewritten.

Patient names use a Unicode human-name policy: at least two letters, with combining marks and internal spaces/apostrophes/hyphens permitted, while digits, punctuation-only values, and control/format characters are rejected. Raw phone structure is validated before normalization: it accepts digits, one optional leading plus, ordinary grouping spaces, at most one balanced digit-only parenthesized group, and non-repeated hyphens only between digits. Dangling, nested, repeated, or abusive separators are rejected rather than stripped. A structurally valid phone must then contain 8-15 digits. An explicit leading `+` is preserved as international identity and its digits may not begin with zero; Armenian `+374` is never inferred for such input. Only no-plus Armenian local forms receive the existing `+374` convenience normalization (eight digits, or nine digits beginning with `0`). Dates must be real Gregorian `YYYY-MM-DD` values (years 0001-9999) and remain subject to the clinic-local same-day/minimum-notice/maximum-horizon rules. Public website bookings default to `pending` unless the clinic explicitly enables automatic confirmation; authenticated staff-created appointments start `confirmed`.

User-entered email addresses share one conservative application syntax contract across public booking, staff-created booking, staff invitation, account login/recovery, and clinic contact editing. Surrounding ASCII spaces are trimmed, but internal whitespace and control/format characters are rejected. The local part uses ordinary ASCII alphanumeric segments separated by a single `.`, `_`, `%`, `+`, or `-`; the domain has at least two valid ASCII labels, no leading/trailing label hyphen, and a 2-63 letter final label. Backend Joi email validation remains an independent syntax/type check with its IANA TLD registry disabled, so reserved/local alphabetic suffixes such as `.local` and `.invalid` follow the same application policy. This is not DNS, mailbox, or deliverability verification.
...
The explicit assigned-care DTO contains only ID, patient name and phone, date/start/end, status and the immutable service name/duration/translations. Phone is included for the assigned-care workflow only. It does not expose email, comments, internal notes, consent, confirmation codes, prices, history, staff actors, lock/quota/idempotency metadata, global patients or medical records. Non-owned detail IDs return 404. Unlinked/inactive profiles fail closed with 403 `DENTIST_PROFILE_REQUIRED`. The initial profile lookup must match the authorization version validated by authentication; assignment/version is checked again after fetching data. A session revoked in either interval cannot receive patient data. The version stamp stays request-private and is not part of the current-user DTO. All existing appointment management routes remain admin/receptionist-only.

`GET`/`PUT /staff/:id/dentist-profile` are admin-only and no-store. PUT accepts exactly `{ dentistId: ObjectId }`, requires a dentist staff account and an existing doctor profile, records a privacy-minimized audit event, and revokes the employee's existing sessions/access version transactionally. A dentist-role account cannot be unlinked; change its role instead. The relationship is included only in admin staff records and remains absent from current-user and public dentist responses. Migration 012 performs a dry-run precondition audit and additively creates a partial unique index so at most one non-archived account can link a profile, including under concurrent invite, assignment, role-change, or restore attempts. There is no email/name guessing or automatic production linking.
