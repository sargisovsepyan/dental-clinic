...
Staff listing applies validated `role`, `isActive`, and `setupComplete` server filters, defaults to page 1/limit 50, and sorts by `name ASC, _id ASC`. Invitations return a safe staff record, never a setup token. Re-inviting a setup-incomplete account issues a new email link but does not change its name/role. Established-account reactivation requires completed setup. Role changes, deactivation, reactivation, and revoke-all-sessions invalidate bearer authorization; self role/deactivation are refused. A successful self session revocation also invalidates the requesting browser.
