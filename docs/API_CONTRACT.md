...
Patient names use a Unicode human-name policy: at least two letters, with combining marks and internal spaces/apostrophes/hyphens permitted, while digits, punctuation-only values, and control/format characters are rejected. Raw phone input accepts only digits, one optional leading plus, spaces, parentheses, and hyphens, then must normalize to 8-15 digits; Armenian local forms are normalized to `+374`. Dates must be real Gregorian `YYYY-MM-DD` values (years 0001-9999) and remain subject to the clinic-local same-day/minimum-notice/maximum-horizon rules. Public website bookings default to `pending` unless the clinic explicitly enables automatic confirmation; authenticated staff-created appointments start `confirmed`.
