...
Patient names use a Unicode human-name policy: at least two letters, with combining marks and internal spaces/apostrophes/hyphens permitted, while digits, punctuation-only values, and control/format characters are rejected. Raw phone structure is validated before normalization: it accepts digits, one optional leading plus, ordinary grouping spaces, at most one balanced digit-only parenthesized group, and non-repeated hyphens only between digits. Dangling, nested, repeated, or abusive separators are rejected rather than stripped. A structurally valid phone must then normalize to 8-15 digits; Armenian local forms are normalized to `+374`. Dates must be real Gregorian `YYYY-MM-DD` values (years 0001-9999) and remain subject to the clinic-local same-day/minimum-notice/maximum-horizon rules. Public website bookings default to `pending` unless the clinic explicitly enables automatic confirmation; authenticated staff-created appointments start `confirmed`.

User-entered email addresses share one conservative application contract across public booking, staff-created booking, staff invitation, account login/recovery, and clinic contact editing. Surrounding ASCII spaces are trimmed, but internal whitespace and control/format characters are rejected. The local part uses ordinary ASCII alphanumeric segments separated by a single `.`, `_`, `%`, `+`, or `-`; the domain has at least two valid ASCII labels, no leading/trailing label hyphen, and a 2-63 letter final label. Backend Joi email validation remains an additional authoritative check.
