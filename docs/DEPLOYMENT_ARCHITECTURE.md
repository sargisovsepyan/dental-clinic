...
Production frontend configuration requires the site and browser API to share an exact non-local HTTPS origin. `src/proxy.ts` remains localization middleware, NOT an API gateway. There is no application open proxy or user-selected upstream. The edge's API upstream is explicit, fixed infrastructure configuration, never a `NEXT_PUBLIC_*` credential or user URL. Server-rendered public GETs use that same public API URL, so web processes need outbound connectivity to the public edge; reserve capacity to avoid an edge-to-frontend-to-edge bottleneck. Do not rewrite `/api/v1/*` back to Next.js.
