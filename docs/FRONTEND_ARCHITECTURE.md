...
`src/api/staff-client.ts` is the authenticated transport boundary. It runtime-validates staff users, appointments, management projections, governed media/consent projections, schedule revisions, bounded conflict metadata, pagination, and availability correlation; sends bearer credentials only to the configured API origin; uses the refresh cookie only on auth endpoints with `credentials: include`; marks protected reads and mutations `no-store`; bounds requests; and surfaces only allowlisted error metadata. Patient fields, provider public IDs from cleanup debt, worker identities, consent history/actors, hidden backend guard fields, and backend error text are not copied into UI models, error objects, or logs.
...
## Governed media and consent administration

Only administrators receive Media / Gallery or Before & After navigation. Direct visits by a receptionist or dentist stop at the signed-in access-denied boundary before any media, cleanup, or consent request is issued; backend authorization remains authoritative and a `403` does not clear the valid staff session.

Gallery creation uses the backend multipart route and its exact JPEG, PNG, WebP, HEIC, or HEIF, 5 MiB envelope. Extension/MIME agreement in the browser is only early UX feedback; backend magic-byte validation remains authoritative. Gallery removal is presented as reversible archive/restore, never as provider deletion. Dentist photos and service images use their dedicated replace/remove routes so unrelated entity fields are not resubmitted. Cleanup debt is a paginated, filterable operational queue; UI models intentionally omit provider public IDs, source record IDs, worker locks, and raw provider detail.

Multipart requests let the browser create the boundary and never set `Content-Type` manually. Single-image operations have a 45-second bound and pair creation has a 60-second bound. Because these backend routes authenticate before multipart parsing, a definitive pre-mutation `401` may perform the existing single-flight refresh and replay exactly once. Network loss, cancellation, or timeout is indeterminate and is never automatically retried. Successful, failed, conflicting, and indeterminate mutations refetch server authority; the UI does not optimistically replace or remove media.

Before/after creation requires two explicitly labeled files and uses the backend's atomic pair route. The UI may submit only the supported consent method, the required confirmation boolean, and an opaque external reference when applicable. Policy version, confirmation time, actor, withdrawal state, and purge state are server-owned. Ordinary unpublish/republish is visibly separate from irreversible consent withdrawal. Withdrawal requires a reason and acknowledgement, immediately fails public visibility closed, and cannot be restored through the UI. Purge is available only after withdrawal, requires the exact backend confirmation phrase and a reason, never auto-retries, clears both image references, and retains the server governance tombstone.

Private before/after models use an explicit positive allowlist and omit consent history, staff actors, provider cleanup identifiers, and patient/appointment data. Public pages continue to use only unauthenticated positive projections with no consent fields. Both public governed before/after responses and all authenticated media responses are `no-store`, preventing shared-cache survival after withdrawal.
...
Playwright uses a localhost-only deterministic mock API. Browser requests to non-local hosts are aborted, the challenge provider is explicitly disabled, and media fixtures resolve only to the repository's local `/og.png`, so no automated test can contact Cloudflare, real Cloudinary, or another external provider. Public/booking coverage remains intact. Staff E2E covers all three roles, HY/RU/EN shells, login/refresh restoration/logout, noindex/storage/cookie checks, appointment operations and conflicts, session expiry, password setup/reset, clinic-management lifecycle/concurrency, governed gallery/entity media, upload failure preservation, media conflicts, atomic pair creation, positive public projection, withdrawal/purge, responsive navigation, and direct-role zero-fetch denials. The runner uses a fresh `.next-e2e` directory for every run to prevent revalidation-cache pollution between scenarios, then removes only that validated generated directory. It owns and terminates only the Next.js and Playwright process trees it starts, including on interrupt and termination signals.
